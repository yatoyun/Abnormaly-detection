...
class mgfn_loss(torch.nn.Module):
    ...
    def forward(self, score_normal, score_abnormal, nlabel, alabel, nor_feamagnitude, abn_feamagnitude):
        ...
        loss_con = self.contrastive(
            torch.norm(abn_feamagnitude, p=1, dim=2), torch.norm(nor_feamagnitude, p=1, dim=2), 1
...
def train(nloader, aloader, balance, model, batch_size, optimizer, device, iterator=0):
    with torch.set_grad_enabled(True):
        model.train()
        ninput, nlabel, macro_video, macro_label = next(nloader)
        ainput, alabel, macro_video, macro_label = next(aloader)
        ...
        weights = torch.tensor([weights[0]] * batch_size + [weights[1]] * batch_size).cuda()

        loss_criterion = mgfn_loss(0.0001, weights)

        cost = (
            loss_criterion(score_normal, score_abnormal, nlabel, alabel, nor_feamagnitude, abn_feamagnitude)
            + loss_smooth
            + loss_sparse
        )
