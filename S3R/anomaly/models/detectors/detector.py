...
class Aggregate(nn.Module):
    def __init__(
        ...
        self.conv_1 = nn.Sequential(
            nn.Conv1d(dim, dim_inner, kernel_size=3, stride=1, dilation=1, padding=1),
            nn.GroupNorm(num_groups=8, num_channels=dim_inner, eps=1e-05),
            nn.ReLU(),
        )
        self.conv_2 = nn.Sequential(
            nn.Conv1d(dim, dim_inner, kernel_size=3, stride=1, dilation=2, padding=2),
            nn.GroupNorm(num_groups=8, num_channels=dim_inner, eps=1e-05),
            nn.ReLU(),
        )
        self.conv_3 = nn.Sequential(
            nn.Conv1d(dim, dim_inner, kernel_size=3, stride=1, dilation=4, padding=4),
            nn.GroupNorm(num_groups=8, num_channels=dim_inner, eps=1e-05),
            nn.ReLU(),
        )
        self.conv_4 = nn.Sequential(
            nn.Conv1d(dim, dim_inner, kernel_size=1, stride=1, padding=0, bias=False),
            nn.ReLU(),
        )
        self.conv_5 = nn.Sequential(
            nn.Conv1d(dim, dim, kernel_size=3, stride=1, padding=1, bias=False),  # TODO: should we keep the bias?
...
class S3R(nn.Module):
    ...
    def __init__(
        ...
        self.video_embedding = nn.Sequential(Aggregate(dim), nn.Dropout(dropout))
        self.macro_embedding = nn.Sequential(Aggregate(dim), nn.Dropout(dropout))

        self.en_normal = enNormal(dim, modality=modality)
        self.de_normal = deNormal(dim, dim // 2, reduction=16)

        self.video_projection = nn.Sequential(
            nn.Conv1d(dim, dim, 3, padding=1), nn.GroupNorm(num_groups=8, num_channels=dim, eps=1e-05), nn.ReLU()
        )
    ...
    def forward(
        ...
        anomaly_score = torch.mean(
            torch.gather(anomaly_scores, 1, idx_abn_score), dim=1
        ...
        regular_score = torch.mean(
            torch.gather(regular_scores, 1, idx_normal_score), dim=1
