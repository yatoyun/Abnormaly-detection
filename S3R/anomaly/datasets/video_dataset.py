...
class Dataset(data.Dataset):
    def __init__(
        ...
        self.data_path_formatter = (
            data_root.joinpath(dataset).joinpath(backbone).joinpath(self.subset).joinpath("{video_id}_{backbone}.npy")
        )
        ...
        if dictionary is None:
            if "universal" in modality:
                ...
                self.dictionary = self._get_video_statistics(self.video_list, memory, regular_file, tmp_dict_file)
    ...
    def _prepare_data(self, video_list: list, verbose: bool = True):
        if self.test_mode is False:
            if "shanghaitech" in self.dataset:
                index = 63
    ...
    def __getitem__(self, index):
        ...
        data_path = data_path_formatter.format(video_id=video_id, backbone=self.backbone)

        features = np.load(data_path, allow_pickle=True)  # tau x N x C, N=10
        features = np.array(features, dtype=np.float32)  # c x N x C
        ...
        if self.test_mode:
            ...
            regular_labels = torch.tensor(0.0)
            features = torch.from_numpy(features).to(self.device)
            features = torch.permute(features, (1, 0, 2))  # N x T x C
            return features, label, dictionary, regular_labels
        else:
    ...
    def process_feat_torch(self, feat, length):
        feat = torch.from_numpy(feat).to(self.device)  # Convert input to a PyTorch tensor
        new_feat = torch.zeros((length, feat.shape[1])).to(self.device)
